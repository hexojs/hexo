...
Tag.prototype.render = async function(str, options, callback) {
  ...
  let result;
  try {
    result = await Promise.fromCallback(cb => { this.env.renderString(str, options, cb); });
  } catch (err) {
    result = await Promise.reject(formatNunjucksError(err, str));
  }
  return result.replace(rPlaceholder, (_, index) => cache[index]);
};
