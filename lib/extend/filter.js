...
Filter.prototype.exec = function(type, data, options = {}) {
  ...
  return Promise.each(this.list(type), filter => Promise.method(filter).apply(ctx, args).then(result => {
...
Filter.prototype.execSync = function(type, data, options = {}) {
  ...
  for (const item of this.list(type)) {
    const result = item.apply(ctx, args);
    args[0] = result == null ? data : result;
  }
