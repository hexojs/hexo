...
module.exports = function(source, data, callback){
  ...
  async.series([
    ...
    function(next){
      data.snippet = data.content.replace(/<(?:.|\n)*?>/gm, '').substr(0, 250) + "...";

      if (typeof data.photos != 'undefined') {
        data.thumb = data.photos[0];
      } else {
        var img_exists = data.content.indexOf("<img src=\"");
        if (img_exists > -1) {
          var i = img_exists + 10;
          data.thumb = data.content.substr(i);
          data.thumb = data.thumb.substr(0, data.thumb.indexOf("\""));
        }
      }
      next();
    }
