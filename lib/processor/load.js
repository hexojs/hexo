...
// return the regex obj which is used to capture the key
var getFilenameRegex = _.memoize(function (key) {
  var configNewPostLink = config.new_post_name,
  regexStr = pathFn.basename(configNewPostLink, pathFn.extname(configNewPostLink));

  Object.keys(filenameKeyMap).forEach(function (keyItr) {
    var value = filenameKeyMap[keyItr];
    value = (keyItr == key) ? '(' + value + ')' : value;
    regexStr = regexStr.replace(keyItr, value);
  });
  return new RegExp(regexStr);
});

// analyze the filename
// return the info from filename
var getInfoFromFilename = function (filename) {
  var result = {};
  Object.keys(filenameKeyMap).forEach(function (keyItr) {
    var regexResult = getFilenameRegex(keyItr).exec(filename);
    if (regexResult) {
      result[keyItr] = regexResult[1];
    }
  });
  return result;
};
...
var load = function(source, callback){
  ...
  async.parallel([
    ...
    var meta = yfm(results[0]),
      stats = results[1],
      filename = pathFn.basename(source, pathFn.extname(source)),
      extname = pathFn.extname(sourcePath).substring(1);

    // get the title and date from filename if defined new_post_name
    if (config.new_post_name){
      var fileinfo = getInfoFromFilename(filename),
      ...
      if (year && month && day) meta.date = meta.date || year + '-' + month + '-' + day + '00:00:00';
    }
