...
Hexo.prototype.watch = function(callback) {
  this._watchBox = _.debounce(() => this._generate({cache: false}), 100);
...
Hexo.prototype._generate = function(options = {}) {
  ...
  // Add routes
    .map(item => {
      ...
      const themeNames = Array.isArray(layout) ? _.uniq(layout) : [layout];
