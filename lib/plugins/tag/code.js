...
module.exports = ctx => function codeTag(args, content) {
  ...
  if (rHighlight.test(arg)) {
    arg = arg.replace(rHighlight, (_match, _enable) => {
      enable = _enable === 'true';
      return '';
    });
  }
  ...
  content = highlight(content, getHighlightConfig(arg, config));
...
function getHighlightConfig(arg, config) {
  let lang = '';
  if (rLang.test(arg)) {
    arg = arg.replace(rLang, (_, _lang) => {
      lang = _lang;
      return '';
    });
  }
  ...
  if (rLineNumber.test(arg)) {
    arg = arg.replace(rLineNumber, (_, _line_number) => {
  ...
  if (rFirstLine.test(arg)) {
    arg = arg.replace(rFirstLine, (_, _first_line) => {
  ...
  let mark = [];
  if (rMark.test(arg)) {
    arg = arg.replace(rMark, (_match, _mark) => {
