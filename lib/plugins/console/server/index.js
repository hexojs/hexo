...
module.exports = function(args, callback){
  var app = express(),
    ...
    password = config.admin_password = args.a || args.admin || config.admin_password || util.uid(8),
    root = config.root,
    base = root + '_/';
  ...
  // CSRF
  app.use(express.csrf(function(req){
  ...
  // Routes
  require('./routes')(new Router(app, base));
  ...
  hexo.post.load({watch: true}, function(err){
    ...
    app.listen(port, function(){
      ...
      log.i('Admin panel\'s password is %s', config.admin_password);
      hexo.emit('server');
    });
  });
};