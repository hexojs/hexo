...
function backtickCodeBlock(data) {
  ...
  const disable_for_lang = config.disable_for_lang || [];
  data.content = data.content.replace(rBacktick, (raw, start, $2, _args, content, end) => {
    ...
    if (args) {
      ...
      if (match) {
        ...
        if (disable_for_lang.includes(options.lang)) {
          return raw;
        }
