...
Box.prototype._dispatch = function(type, path, callback){
  ...
  hexo.log.log(type, 'Processing: %s', path);
...
var _loadFileList = function(base, callback){
  var Cache = hexo.model('Cache'),
    cache = {};

  Cache.each(function(item){
    if (item.full_source.substring(0, base.length) === base){
      cache[item.full_source.substring(base.length)] = item;
    }
  });
  ...
  async.auto({
    list: function(next){
      file.list(base, function(err, files){
        if (err) return next(err);

        created = _.difference(files, keys);
        deleted = _.difference(keys, files);

        next();
      });
    },
    create: ['list', function(next){
      async.each(created, function(item, next){
        var source = pathFn.join(base, item);

        fs.stat(source, function(err, stats){
          if (err) return next(err);

          Cache.insert({full_source: source, mtime: stats.mtime.getTime()}, function(){
    ...
    update: ['list', function(next){
      async.each(_.difference(keys, deleted), function(item, next){
        var source = pathFn.join(base, item),
          data = cache[item];
