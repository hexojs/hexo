...
import {
  expected,
  content,
  expected_disable_nunjucks,
  content_for_issue_3346,
  expected_for_issue_3346,
  content_for_issue_4460
} from '../../fixtures/post_render';
...
describe('Post', () => {
  ...
  before(async () => {
    ...
    defaultCfg = deepClone(hexo.config);
  });
  ...
  afterEach(() => {
    hexo.config = deepClone<typeof hexo.config>(defaultCfg as any);
  });
  ...
  it('create()', async () => {
    ...
    const content
      = ['---', 'title: Hello World', 'date: ' + date.format('YYYY-MM-DD HH:mm:ss'), 'tags:', '---'].join('\n') + '\n';
  ...
  it('create() - extra data', async () => {
    ...
    const content
      = ['---', 'title: Hello World', 'foo: bar', 'date: ' + date.format('YYYY-MM-DD HH:mm:ss'), 'tags:', '---'].join(
        '\n'
      ) + '\n';
  ...
  it('create() - rename if target existed', async () => {
    ...
    await Promise.all([unlink(path), unlink(join(hexo.source_dir, '_posts', 'Hello-World.md'))]);
  });
  ...
  it('create() - follow the separator style in the scaffold', async () => {
    ...
    result.content.should.eql(['---', 'title: Hello World', '---'].join('\n') + '\n');
  ...
  it('create() - wrap with quote when necessary - yaml tag', async () => {
    ...
    const data = await readFile(result.path);
    data.should.eql(['---', 'title: \'!!js/regexp /pattern/gim\'', '---'].join('\n') + '\n');
  ...
  it('create() - JSON front-matter', async () => {
    ...
    result.content.should.eql(['"title": "Hello World",', '"lang": "en"', ';;;'].join('\n') + '\n');
  ...
  it('publish() - replace existing files', async () => {
    ...
    await Promise.all([post.create({ title: 'Hello World', layout: 'draft' }), post.create({ title: 'Hello World' })]);
    const data = await post.publish(
      {
        slug: 'Hello-World'
      },
  ...
  it('publish() - asset folder', async () => {
    ...
    await Promise.all([writeFile(join(assetDir, 'a.txt'), 'a'), writeFile(join(assetDir, 'b.txt'), 'b')]);
    const result = await post.publish({
      slug: 'Hello-World'
    });
  ...
  it('render() - skip render phase if it\'s nunjucks file', async () => {
    const content = ['{% quote Hello World %}', 'quote content', '{% endquote %}'].join('\n');
  ...
  it('render() - escaping nunjucks blocks with similar names', async () => {
    ...
    const content = ['{% codeblock %}', code, '{% endcodeblock %}', '', '{% code %}', code, '{% endcode %}'].join('\n');
  ...
  it('render() - allow backtick code block in "blockquote" tag plugin', async () => {
    ...
    const content = ['{% blockquote %}', '```', code, '```', '{% endblockquote %}'].join('\n');

    const data = await post.render('', {
      content
    });
    data.content.trim().should.eql(['<blockquote>' + highlighted + '</blockquote>'].join('\n'));
  });
  ...
  it('render() - backtick cocde block in blockquote', async () => {
    ...
    data.content
      .trim()
      .should.eql(
        ['<p>Hello</p>', '<blockquote>', '<p>This is a code-block</p>', highlighted + '</blockquote>'].join('\n')
      );
  });
  ...
  it('render() - backtick cocde block (followed by a paragraph) in blockquote', async () => {
    const code = 'alert("Hello world")';
    const highlighted = highlight(code);
    const quotedContent = ['This is a code-block', '', '```', code, '```', '', 'This is a following paragraph'];
  ...
  it('render() - blank lines in backtick cocde block in blockquote', async () => {
    ...
    const quotedContent = ['This is a code-block', '', '> ```', ...code2, '```', '', 'This is a following paragraph'];
  ...
  it('render() - markdown content right after swig tag', async () => {
    ...
    data.content
      .trim()
      .should.include('<h1 id="Title-0"><a href="#Title-0" class="headerlink" title="Title 0"></a>Title 0</h1>');
    data.content
      .trim()
      .should.include('<h1 id="Title-1"><a href="#Title-1" class="headerlink" title="Title 1"></a>Title 1</h1>');
    data.content
      .trim()
      .should.include('<h1 id="Title-2"><a href="#Title-2" class="headerlink" title="Title 2"></a>Title 2</h1>');
    data.content
      .trim()
      .should.include('<h1 id="Title-3"><a href="#Title-3" class="headerlink" title="Title 3"></a>Title 3</h1>');
  });
  ...
  it('render() - "{{" & "}}" inside inline code', async () => {
    ...
    data.content
      .trim()
      .should.eql(
        `<p>In Go’s templates, blocks look like this: <code>${escapeSwigTag('{{block "template name" .}} (content) {{end}}')}</code>.</p>`
      );
  });
  ...
  it('render() - issue #3543', async () => {
    ...
    const content = ['```js', js, '```', '{% raw %}', '<p>Foo</p>', '{% endraw %}', '```html', html, '```'].join('\n');
  ...
  it('render() - issue #4087', async () => {
    ...
    data.content
      .trim()
      .should.contains(`<pre><code>${escapeSwigTag('{% pullquote %}foo foo foo{% endpullquote %}')}\n</code></pre>`);
  ...
  it('render() - no double escape in code block (issue #4385)', async () => {
    const content = ['```rust', 'fn main() {', '    println!("Hello, world!");', '}', '```'].join('\n');
  ...
  it('render() - tags in different lines', async () => {
    const content = ['{% link', 'foobar', 'https://hexo.io/', 'tttitle', '%}'].join('\n');
